...
if __name__ == '__main__':
    ...
    # 方法1: 直接通过列名列表选择（最常用）
    #selected_cols = ['代码', '名称', '今开','最新价','量比']
    #df1 = df[selected_cols]
    #df=df1.rename(columns={'代码': 'code', '名称': 'name', '今开': 'open','最新价':'current','量比':'volume_ratio'})

    df = df[df['名称'].apply(lambda x: 'ST' not in str(x) and '*ST' not in str(x) and 'PT' not in str(x)
                                       and '退' not in str(x) )]

    df = df[df['代码'].apply(lambda x: not str(x)>'699999')]

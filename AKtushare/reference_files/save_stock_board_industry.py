...
if __name__ == '__main__':
    ...
    df = pd.DataFrame(stock_board_industry_summary_ths_df)
    ...
    plt.rcParams["mathtext.fontset"] = "custom"  # 公式

    # 画条形图
    plt.figure(figsize=(10, 6))
    plt.barh(top_sectors['板块'], top_sectors['涨跌幅'])
    plt.title('行业板块涨幅TOP10')
    plt.show()

    # 存储到SQLite数据库
    df_to_sqlite(
        df=df,
        table_name='stock_board_industry_summary',
        db_name='akshare.db',
        if_exists='replace'
    )

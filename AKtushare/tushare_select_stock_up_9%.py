...
if __name__ == "__main__":
    ...
    df2 = df_daybf2[["ts_code", "close"]]
    df2.columns = ["ts_code", "close_bf2"]

    df = pd.merge(df1, df2, on="ts_code", how="left")
    ...
    df = df.drop(df[df["close_bf1"] - df["close_bf2"] < 0.095 * df["close_bf2"]].index)
    ...
    df_to_sqlite(
        df=df,
        table_name="tushare_select_up_9%",
        db_name=db_path,
        if_exists="replace",
    )
